...
const SOSReports: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setLoading(true);
      try {
        const data = await adminSOSAPI.getSOSReports();
        setReports(data);
        // Analytics calculation
  const solved = data.filter((r: SOSReport) => r.status === 'solved').length;
  const inProgress = data.filter((r: SOSReport) => r.status === 'in-progress').length;
  const pending = data.filter((r: SOSReport) => !r.status || r.status === 'pending').length;
  ...
  return (
    <div className="max-w-5xl mx-auto py-8">
      <h1 className="text-3xl font-bold mb-6 flex items-center gap-2 text-red-600"><AlertTriangle /> SOS Reports</h1>
      {/* Analytics */}
